...
1. Close the trace session:
   ...
   ```
### Viewing traces using Pktmon

An alternate method for viewing traces either in a log file or in real-time is available through `pktmon.exe` included in Windows.  Pktmon provides the additional capability of  capturing packets as they traverse the network components in the Windows Operating System.  You can included messages from additional providers, such as those provided by eBPF in the capture output.

To view only the eBPF trace events in real-time using `pktmon.exe`:
```cmd
pktmon start --trace -p "{394f321c-5cf4-404c-aa34-4df1428a7f9c}" -l 5 -p "{f2f2ca01-ad02-4a07-9e90-95a2334f3692}" -l 5 -p "{00e7ee66-5b24-5c41-22cb-af98f63e2f90}" -l 5 -m real-time
```
Press ctrl-c to stop the real-time output.

To view only the output generated by the [bpf_printk](https://microsoft.github.io/ebpf-for-windows/bpf__helper__defs_8h.html#aae337e68db96b4b9470f8c519386cbec) helper:
```cmd
pktmon start --trace -p "{394f321c-5cf4-404c-aa34-4df1428a7f9c}" -l 5 -m real-time
```

To correllate packet capture with eBPF trace events in real-time:

1. (Optional) Add a filter to reduce the number of packets captured.  In this example we will only capture ICMP messages:
   ```cmd
   pktmon filter add -t ICMP
   ```
2. Begin capture of packets and eBPF events and send the output to the console in real-time:
   ```cmd
   pktmon start --capture --trace -p "{394f321c-5cf4-404c-aa34-4df1428a7f9c}" -l 5 -p "{f2f2ca01-ad02-4a07-9e90-95a2334f3692}" -l 5 -p "{00e7ee66-5b24-5c41-22cb-af98f63e2f90}" -l 5 -m real-time 
   ```
3. Press ctrl-c to stop the capture

To correllate packet capture with eBPF trace events and store to a file for analysis:

1. (Optional) Add a filter to reduce the number of packets captured.  In this example we will only capture ICMP messages:
   ```cmd
   pktmon filter add -t ICMP
   ```
2. Begin capture of packets and eBPF events and send the output to an etl file:
   ```cmd
   pktmon start --capture --trace -p "{394f321c-5cf4-404c-aa34-4df1428a7f9c}" -l 5 -p "{f2f2ca01-ad02-4a07-9e90-95a2334f3692}" -l 5 -p "{00e7ee66-5b24-5c41-22cb-af98f63e2f90}" -l 5 -m circular
   ```
3. Exercise the scenario for which you want to capture the packets and events.
4. Stop the capture of packets and events:
   ```cmd
   pktmon stop
   ```
5. Convert the output to text:
   ```cmd
   pktmon etl2txt pktmon.etl
   ```
6. Open pktmon.txt in the editor of your choice for review.  This snippet shows a single ICMP packet arriving in the system, being sent to the XDP hook which does nothing but print "Hello World!", and passing it on for further processing by the system.
   ```
   [00]0000.0000::2023-10-16 19:32:59.414370400 [Microsoft-Windows-PktMon] PktGroupId 5, PktNumber 1, Appearance 1, Direction Rx , Type Ethernet , Component 10, Edge 1, Filter 1, OriginalSize 74, LoggedSize 74 
	02-00-00-00-00-05 > 04-00-00-00-00-07, ethertype IPv4 (0x0800), length 74: 10.3.0.9 > 10.2.3.5: ICMP echo request, id 1, seq 9250, length 40
   [00]0000.0000::2023-10-16 19:32:59.414373200 [Microsoft-Windows-PktMon] PktGroupId 5, PktNumber 1, Appearance 2, Direction Rx , Type Ethernet , Component 19, Edge 2, Filter 1, OriginalSize 74, LoggedSize 74 
   	02-00-00-00-00-05 > 04-00-00-00-00-07, ethertype IPv4 (0x0800), length 74: 10.3.0.9 > 10.2.3.5: ICMP echo request, id 1, seq 9250, length 40
   [00]0000.0000::2023-10-16 19:32:59.414397300 [EbpfForWindowsProvider] Event: EbpfGenericMessage, Message: Hello World!
   [00]0000.0000::2023-10-16 19:32:59.414401100 [EbpfForWindowsProvider] Event: EbpfSuccess, Message: _ebpf_link_instance_invoke_batch returned success
   [00]0000.0000::2023-10-16 19:32:59.414411800 [Microsoft-Windows-PktMon] PktGroupId 5, PktNumber 1, Appearance 3, Direction Rx , Type Ethernet , Component 19, Edge 1, Filter 1, OriginalSize 74, LoggedSize 74 
   	02-00-00-00-00-05 > 04-00-00-00-00-07, ethertype IPv4 (0x0800), length 74: 10.3.0.9 > 10.2.3.5: ICMP echo request, id 1, seq 9250, length 40
   [00]0000.0000::2023-10-16 19:32:59.414415300 [Microsoft-Windows-PktMon] PktGroupId 5, PktNumber 1, Appearance 4, Direction Rx , Type Ethernet , Component 41, Edge 1, Filter 1, OriginalSize 74, LoggedSize 74 
   	02-00-00-00-00-05 > 04-00-00-00-00-07, ethertype IPv4 (0x0800), length 74: 10.3.0.9 > 10.2.3.5: ICMP echo request, id 1, seq 9250, length 40
   [00]0000.0000::2023-10-16 19:32:59.414511400 [Microsoft-Windows-PktMon] Drop: PktGroupId 5, PktNumber 1, Appearance 5, Direction Rx , Type IP , Component 46, Filter 1, DropReason ICMP: inspection drop , DropLocation 0xE0004199, OriginalSize 60, LoggedSize 60 
   	Drop: ip: 10.2.3.5 > 10.3.0.9: ICMP echo request, id 1, seq 9250, length 40
   ```
